...
- Added `oxrsys-headset-helper`, a native process that owns the wired headset: it captures the panel's display so no other window can appear on it, runs the driver and tracking, shows a head-tracked lobby between sessions, and receives game frames zero-copy over shared IOSurfaces. The runtime's wired backend is now a socket client of it (no USB, driver or AppKit in the game process), which makes wired headsets work from x86_64 Wine processes.
...
- Added 6DoF head tracking for the wired headset through Monado's SLAM tracker and a Basalt VIT plugin: `drivers/tools/build_basalt.sh` builds `libbasalt.dylib`, the headset helper loads it when found (`wired_vit_library`, `--vit-library`, `OXRSYS_VIT_LIBRARY`; `wired_position_tracking = false` keeps IMU-only tracking), runs it with Basalt's WMR pipeline profile and a calibration file generated from the headset's own calibration, holds the head at the fixed eye height until the estimate has settled, and the runtime learns the tracking kind and position flag from `HeadsetInfo`. The runtime now also passes `wired_display_id` and `wired_eye_height_m` to the helper. Also `oxrsys_wmr_probe --snapshot DIR` (one frame per tracking camera) and a camera tap for observing frames next to the trackers.
- Added a Headset section to the SwiftUI Home and Qt Home Streaming tabs that selects between streaming clients and a wired Windows Mixed Reality headset (`wired_headset`), edits `wired_eye_height_m` and `wired_display_id`, and shows the wired setup notes (USB + video cable, EDID override, Bluetooth motion controllers or PS Move). Runtime activity now labels the `wired` transport and `wmr` device type.
- Added positional tracking for 1st-gen Windows Mixed Reality motion controllers from the headset cameras (`drivers/monado/wmr_controller_tracking.c`, OpenCV builds): the helper sends the controllers the LED timesync and keepalive packets upstream Monado lacks, finds LED blobs in the headset's short-exposure controller frames, solves controller poses with Monado's constellation tracker, keeps only poses whose gravity direction agrees with the controller's accelerometer (the mirrored left/right rings otherwise swap hands), and uses a recent optical position instead of the arm model (`--no-controller-tracking` turns it off). The camera monitor draws an `L`/`R` box around each identified controller, dashed boxes around unassigned LED clusters, and per-hand visibility; `OXRSYS_HEADSET_MONITOR_LED_FRAMES=1` shows the LED frames. Plus `oxrsys_wmr_ct_selftest`, which checks the tracker setup on a synthetic constellation.
- Added a camera monitor to `oxrsys-headset-helper` (`--monitor` / `OXRSYS_HEADSET_MONITOR=1`): a desktop window showing both headset tracking cameras with the features Basalt tracks (depth-coloured, tracked vs. new), the sphere-tracked PS Move projected into camera 0, and tracking status. The features come through `liboxrsys-vit-monitor.dylib`, a VIT plugin shim that forwards to the real library and records the pose features.
