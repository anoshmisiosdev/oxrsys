...
- Fixed head tracking being dropped entirely over the USB/ADB transport for clients built before the aim-pose fields were appended to `TrackingPacket`. The TCP tracking read loop gated on `payload.size() >= sizeof(TrackingPacket)`, so when the struct grew from 1008 to 1064 bytes every record from an older client was silently discarded: the connection reported "tracking client connected", but no packet was ever stored, `TrackingReceiver::IsReceiving()` stayed false, `InputManager::UpdateFromStreaming()` never ran, and `xrLocateSpace` served the default head pose forever (no 6DoF). All transports now share `oxr::protocol::IsAcceptableTrackingPayloadSize()`, which accepts any packet down to `TRACKING_PACKET_MIN_WIRE_SIZE` and zero-fills the newer fields. The same stale check in the wired-headset reader is fixed too.
