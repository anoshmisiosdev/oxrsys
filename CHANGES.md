...
- Added 6DoF head tracking for the wired headset through Monado's SLAM tracker and a Basalt VIT plugin: `drivers/tools/build_basalt.sh` builds `libbasalt.dylib`, the headset helper loads it when found (`--vit-library`, `OXRSYS_VIT_LIBRARY`) and otherwise stays on IMU orientation, and the runtime learns the tracking kind and position flag from `HeadsetInfo`. Also `oxrsys_wmr_probe --snapshot DIR` (one frame per tracking camera) and a camera tap for observing frames next to the trackers.
