...
#define OXRSYS_HELPER_IPC_WANT_MACH 1
...
using oxrsys::helper_ipc::PortMsg;
using oxrsys::helper_ipc::PortMsgRecv;
using oxrsys::helper_ipc::kMsgIdChildPort;
using oxrsys::helper_ipc::kMsgIdSurface;
using oxrsys::helper_ipc::Reader;
...
{
    uint8_t hdr[oxrsys::helper_ipc::kHeaderBytes];
    ...
    if (magic != oxrsys::helper_ipc::kMagic || version != kProtocolVersion ||
        len > oxrsys::helper_ipc::kMaxPayloadBytes)
    {
        return false;
    }
    payload.resize(len);
...
bool WiredHeadset::EnsureOpen(const ConfigValues& config)
{
    ...
    // Hello, then wait for the headset description.
    {
        ...
        oxrsys::helper_ipc::PutU16(p, kProtocolVersion);
        oxrsys::helper_ipc::PutU32(p, (uint32_t)getpid());
...
bool WiredHeadset::AttachGraphics(const GraphicsContext& graphicsContext)
{
    ...
    {
        std::vector<uint8_t> p;
        oxrsys::helper_ipc::PutU32(p, kSlots);
    ...
    for (uint32_t i = 0; i < kSlots; i++)
    {
        ...
        msg.width = width;
        msg.height = height;
        msg.pixelFormat = oxrsys::helper_ipc::kPixelFormatBGRA;
