...
private:
    ...
    // Under x86_64/Rosetta without the encoder helper the HEVC encode runs in
    // software (~30-40ms/frame), far longer than the 11-14ms frame period at
    // 72-90Hz. With only a few slots, any callback-delivery jitter fills every
    // slot and forces a drop cascade at AcquireSlot. More in-flight slots absorb
    // that jitter; hardware encode (helper or native arm64) finishes in ~8ms and
    // needs far fewer, but the extra slots are cheap headroom there too.
