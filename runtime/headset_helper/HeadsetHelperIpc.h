...
inline void PutF32(std::vector<uint8_t>& b, float v)
{
    ...
    helper_ipc::PutU32(b, u);
}

inline float GetF32(helper_ipc::Reader& r)
{
    uint32_t u = r.U32();
    float v;
...
{
    helper_ipc::PutU32(b, (uint32_t)s.size());
    b.insert(b.end(), s.begin(), s.end());
}

inline std::string GetString(helper_ipc::Reader& r)
{
    uint32_t len = r.U32();
    if (len > 4096) { return {}; }
...
inline std::vector<uint8_t> EncodeHeadsetInfo(const HeadsetInfo& info)
{
    ...
    helper_ipc::PutU32(p, info.panelW);
    helper_ipc::PutU32(p, info.panelH);
    helper_ipc::PutU32(p, info.eyeW);
    helper_ipc::PutU32(p, info.eyeH);
    for (float f : info.fov) PutF32(p, f);
    helper_ipc::PutU32(p, info.refreshHz);
    helper_ipc::PutU32(p, (info.displayReady ? 1u : 0u) | (info.positionTracked ? 2u : 0u));
...
{
    helper_ipc::Reader r(payload.data(), payload.size());
    info.panelW = r.U32();
    info.panelH = r.U32();
    info.eyeW = r.U32();
    info.eyeH = r.U32();
...
{
    std::vector<uint8_t> p;
    helper_ipc::PutU32(p, f.slot);
    helper_ipc::PutI64(p, f.timestampNs);
    helper_ipc::PutU32(p, f.hasPose ? 1u : 0u);
...
inline std::vector<uint8_t> Frame(MsgType type, const std::vector<uint8_t>& payload)
{
    ...
    out.reserve(helper_ipc::kHeaderBytes + payload.size());
    helper_ipc::PutU32(out, helper_ipc::kMagic);
    helper_ipc::PutU16(out, (uint16_t)type);
    helper_ipc::PutU16(out, kProtocolVersion);
    helper_ipc::PutU32(out, (uint32_t)payload.size());
