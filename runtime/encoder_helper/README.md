...
The helper implements HEVC Main (8-bit) only. It is skipped — with a log line —
when the negotiated codec is H.264 and when `encoder_10bit` selects HEVC Main10;
those keep using the in-process session.

The runtime log then shows:
`VideoEncoder: hardware HEVC via the native-arm64 out-of-process helper` and
