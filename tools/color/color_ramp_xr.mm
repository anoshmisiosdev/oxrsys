...
/*!
 * @file
 * @brief Colour-pipeline test pattern: known sRGB code values streamed through the runtime.
 *
 * Renders, into an sRGB (MTLPixelFormatRGBA8Unorm_sRGB = 71) swapchain for both eyes, a 16-step
 * grey ramp (codes 0, 17, ... 255) over 8 patches (codes 0, 16, 32, 64, 128, 192, 235, 255).
 * Compare those codes with the encoded stream (tools/color/hevc_color_probe.mm, with
 * OXRSYS_DEBUG_DUMP_STREAM from the encoder debug knobs) and with `adb exec-out screencap -p`
 * on the headset.
 *
 * Build (x86_64, like the runtime):
 *   clang++ -arch x86_64 -std=c++17 -fobjc-arc -O1 -DXR_USE_GRAPHICS_API_METAL \
 *     -I build/x86/_deps/openxr-src/include tools/color/color_ramp_xr.mm -o color_ramp \
 *     -framework Metal -framework Foundation -L build/x86/_deps/openxr-build/src/loader \
 *     -lopenxr_loader -Wl,-rpath,build/x86/_deps/openxr-build/src/loader
 * Run: XR_RUNTIME_JSON=build/x86/runtime/oxrsys-runtime.json ./color_ramp --seconds 60
 */
...
struct Uniforms { float4 tan_fov; packed_float3 tint; float time; };
...
static float srgb_to_linear(float c) { return c <= 0.04045 ? c / 12.92 : pow((c + 0.055) / 1.055, 2.4); }
// Top half: 16-step grey ramp, code 0..255 (step 17). Bottom half: 8 patches with codes
// 0, 16, 32, 64, 128, 192, 235, 255. Values are sRGB code values; the target is an sRGB format,
// so the shader outputs the linear value that encodes to exactly that code.
fragment float4 fs(Out in [[stage_in]], constant Uniforms &u [[buffer(0)]]) {
    float code;
    if (in.uv.y < 0.5) {
        code = floor(in.uv.x * 16.0) * 17.0;
    } else {
        const float patches[8] = {0.0, 16.0, 32.0, 64.0, 128.0, 192.0, 235.0, 255.0};
        code = patches[min(int(in.uv.x * 8.0), 7)];
    }
    float v = u.time > 0.5 ? srgb_to_linear(code / 255.0) : code / 255.0;
...
main(int argc, char **argv)
{
	...
	@autoreleasepool {
		...
		strncpy(ici.applicationInfo.applicationName, "oxrsys_color_ramp", XR_MAX_APPLICATION_NAME_SIZE - 1);
		...
		int64_t format = formats.front();
		for (int64_t f : formats) if (f == 71) format = 71; // MTLPixelFormatRGBA8Unorm_sRGB, as Hitman's swapchains
		printf("Swapchain format %lld (%s)\n", (long long)format, format == 71 || format == 81 ? "sRGB" : "UNORM");
		...
		while (!exitRequested) {
			...
			for (uint32_t i = 0; i < viewCount; i++) {
				...
				u.time = (format == 71 || format == 81) ? 1.0f : 0.0f;
