...
(see [Windows Mixed Reality headsets](platforms/wmr.md)), which needs `brew install hidapi libusb`
for a host-native build. A cross-architecture build (for example the x86_64 runtime loaded by a
Rosetta Wine host) or `-DOXRSYS_WMR_BUNDLED_USB=ON` fetches and builds hidapi and libusb from
source instead. Pass `-DOXRSYS_BUILD_WMR_DRIVER=OFF` to skip the driver. The headset helper
(`runtime/headset_helper/`) is only built host-native.
